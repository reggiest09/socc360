...
# soccer camp prices
  ...
  def stripe_wffull_amount(user)
    (135 * user.child_count.to_i) * 100
  end
 def stripe_wffhalf_amount(user)
    (109 * user.child_count.to_i) * 100
  end

   def stripe_nonhalf_amount(user)
    (139 * user.child_count.to_i) * 100
  end
  def stripe_nonfull_amount(user)
    (169 * user.child_count.to_i) * 100
  end
